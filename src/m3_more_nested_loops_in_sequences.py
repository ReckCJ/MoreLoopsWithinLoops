...
def run_test_largest_number():
    ...
    expected = 5000
    answer = largest_number(([], [], [], (5, 50, 500, 5000, 500, 50, 5)))
    print('Expected and actual are:', expected, answer)
...
def run_test_largest_negative_number():
    ...
    # Test 1:
    expected = -4
    answer = largest_negative_number([[1, 2, 3, -4, -5], (10, -20, 30), (-720, -1000)])
    print('Expected and actual are:', expected, answer)

    # Test 2:
    expected = -1
    answer = largest_negative_number(([-3, -5, -7], [-1, 1, -2, 2], [0, 1, 1, 2]))
    print('Expected and actual are:', expected, answer)

    # Test 3:
    expected = -1
    answer = largest_negative_number(([-1, -2, 3, 4], (-5, -50, -500, -5000)))
    print('Expected and actual are:', expected, answer)
        ...
        if len(seq_seq[k]) >= 1:
            for j in range(len(seq_seq[k])):
                if seq_seq[k][j] < 0:
                    largest = seq_seq[k][j]
                    break
    for g in range(len(seq_seq)):
        for h in range(len(seq_seq[g])):
            if largest < seq_seq[g][h] < 0:
                largest = seq_seq[g][h]
    return largest
